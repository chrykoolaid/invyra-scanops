...
const DEMO_REMOTE_ACTORS = {
  staff2: {
    ...
    shiftId: "shift_demo_morning_001",
  ...
  return [
    {
      ...
      helper: "You · local/demo device alias",
    },
    {
      deviceId: "HH-002",
      deviceLabel: "HH-002",
      userId: DEMO_REMOTE_ACTORS.staff2.userId,
      userName: DEMO_REMOTE_ACTORS.staff2.userName,
      role: DEMO_REMOTE_ACTORS.staff2.role,
      sessionId: DEMO_REMOTE_ACTORS.staff2.sessionId,
      shiftId: DEMO_REMOTE_ACTORS.staff2.shiftId,
      shiftLabel: DEMO_REMOTE_ACTORS.staff2.shiftLabel,
      status: "Active",
      isCurrentDevice: false,
      helper: "Local/demo participant",
    },
    {
      deviceId: "HH-003",
      deviceLabel: "HH-003",
      userId: DEMO_REMOTE_ACTORS.supervisor.userId,
      userName: DEMO_REMOTE_ACTORS.supervisor.userName,
      role: DEMO_REMOTE_ACTORS.supervisor.role,
      sessionId: DEMO_REMOTE_ACTORS.supervisor.sessionId,
      shiftId: DEMO_REMOTE_ACTORS.supervisor.shiftId,
      shiftLabel: DEMO_REMOTE_ACTORS.supervisor.shiftLabel,
      status: "Active",
      isCurrentDevice: false,
      helper: "Local/demo participant",
    },
    {
      deviceId: "HH-004",
      deviceLabel: "HH-004",
      userId: DEMO_REMOTE_ACTORS.manager.userId,
      userName: DEMO_REMOTE_ACTORS.manager.userName,
      role: DEMO_REMOTE_ACTORS.manager.role,
      sessionId: DEMO_REMOTE_ACTORS.manager.sessionId,
      shiftId: DEMO_REMOTE_ACTORS.manager.shiftId,
      shiftLabel: DEMO_REMOTE_ACTORS.manager.shiftLabel,
      status: "Active",
      isCurrentDevice: false,
      helper: "Local/demo participant",
    },
...
function ownedTask(owner, input = {}) {
  return baseTask({
    ...
    lockReason: input.lockReason || "Task is claimed by another local/demo device.",
  });
}
...
function defaultState(context = getScanOpsGovernanceContext()) {
  ...
  const initialEvents = [
    makeEvent("DEVICE_JOINED_SESSION", { eventLabel: `${current.userName} joined store operations session`, eventType: COLLABORATION_STATES.DEVICE_JOINED_SESSION }, context),
    makeEvent("TASK_CLAIMED", {
      eventLabel: "Staff 2 claimed Markdown Batch #004",
      eventType: COLLABORATION_STATES.TASK_CLAIMED,
      taskId: "task_markdown_batch_004",
      taskType: TASK_TYPES.MARKDOWN_APPROVAL,
      sourceWorkflow: "Markdown Approval",
      previousOwnerUserId: null,
      newOwnerUserId: DEMO_REMOTE_ACTORS.staff2.userId,
    }, context),
    makeEvent("TASK_CONFLICT_DETECTED", {
      eventLabel: "Waste Review #002 quantity mismatch captured",
      eventType: COLLABORATION_STATES.TASK_CONFLICT_DETECTED,
      taskId: "task_waste_conflict_002",
  ...
  return {
    collaborationVersion: COLLABORATION_VERSION,
    session: {
      collaborationSessionId: "collab_store_ops_session_demo_001",
      state: COLLABORATION_STATES.COLLAB_SESSION_ACTIVE,
      label: "Store Ops Session",
      mode: "Local collaboration preview",
    ...
    tasks: [
      baseTask({
        taskId: "task_replenish_aisle_3",
        taskType: TASK_TYPES.REPLENISHMENT,
        taskLabel: "Replenish Aisle 3",
        taskSummary: "Backroom-to-shelf fill task ready to claim.",
        sourceWorkflow: "Replenishment",
        sourceRecordId: "repl_demo_aisle_3",
        ownershipStatus: COLLABORATION_STATES.TASK_AVAILABLE,
        createdAt,
      }),
      ownedTask(DEMO_REMOTE_ACTORS.staff2, {
        taskId: "task_markdown_batch_004",
        taskType: TASK_TYPES.MARKDOWN_APPROVAL,
        taskLabel: "Markdown Batch #004",
        taskSummary: "Short-dated grocery markdown review.",
        sourceWorkflow: "Markdown Approval",
        sourceRecordId: "md_batch_004",
        createdAt,
      }),
      baseTask({
        taskId: "task_waste_conflict_002",
        taskType: TASK_TYPES.WASTE_REVIEW,
        taskLabel: "Waste Conflict #002",
        taskSummary: "HH-001 submitted 2 damaged units; HH-002 submitted 5 units.",
        sourceWorkflow: "Waste Review",
        sourceRecordId: "waste_conflict_002",
        ownershipStatus: COLLABORATION_STATES.TASK_CONFLICT_REVIEW_REQUIRED,
        conflictStatus: COLLABORATION_STATES.TASK_CONFLICT_REVIEW_REQUIRED,
        conflictReason: COLLABORATION_CONFLICT_TYPES.QUANTITY_MISMATCH,
        conflictRecordIds: ["waste_submit_hh001", "waste_submit_hh002"],
        conflictRisk: "HIGH",
        lockMode: "CONFLICT_REVIEW_LOCK",
        lockReason: "Manager review required before sync.",
        createdAt,
      }),
      baseTask({
        taskId: "task_shelf_ticket_batch_013",
        taskType: TASK_TYPES.SHELF_TICKET,
        taskLabel: "Shelf Ticket Batch #013",
        taskSummary: "Missing shelf label batch ready for ownership.",
        sourceWorkflow: "Shelf Tickets",
        sourceRecordId: "shelf_ticket_batch_013",
        ownershipStatus: COLLABORATION_STATES.TASK_AVAILABLE,
        createdAt,
      }),
      ownedTask(DEMO_REMOTE_ACTORS.supervisor, {
        taskId: "task_waste_review_009",
        taskType: TASK_TYPES.WASTE_REVIEW,
        taskLabel: "Waste Review #009",
        taskSummary: "Low-risk waste review owned by Supervisor Preview.",
        sourceWorkflow: "Waste Review",
        sourceRecordId: "waste_review_009",
        createdAt,
      }),
    ],
...
export function claimCollaborationTask(taskId) {
  ...
  return persistTaskAction(task.taskId, COLLABORATION_STATES.TASK_CLAIMED, {
    ...
    lockReason: "Claimed by current local/demo device.",
  }, {
...
export function registerCollaborationTaskForRecord({ taskId, taskType, taskLabel, taskSummary, sourceWorkflow, sourceRecordId, ownerMode = "current", conflictRisk = "LOW" } = {}) {
  ...
  const ownerPatch = ownerMode === "current" ? {
    ...
    claimedAt: nowIso(),
    lockMode: "SOFT_OWNER_LOCK",
    lockReason: "Created and owned by current local/demo device.",
  } : {};
  const task = baseTask({
