...
export default function OfflineBanner() {
  ...
  if (isOnline && flushing) {
    return (
      <div className="border-b border-primary/20 bg-primary/5 px-4 py-2.5" role="status" aria-live="polite">
        <div className="flex items-center gap-2">
          <RefreshCw className="h-4 w-4 shrink-0 animate-spin text-primary" aria-hidden="true" />
  ...
  if (showSynced && isOnline) {
    return (
      <div className="border-b border-accent/20 bg-accent/10 px-4 py-2.5" role="status" aria-live="polite">
        <div className="flex items-center gap-2">
  ...
  if (isOnline && queueCount > 0) {
    const syncLabel = `Sync ${queueCount} waiting scan record${queueCount === 1 ? "" : "s"} now`;
    return (
      <div className="border-b border-amber-200 bg-amber-50 px-4 py-2.5" role="status" aria-live="polite">
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={manualFlush} className="shrink-0 flex items-center gap-1.5 rounded-xl bg-amber-600 px-3 py-1.5 text-xs font-black text-white active:opacity-80" aria-label={syncLabel} title={syncLabel}>
