...
import { Activity, BadgeHelp, BadgePercent, BarChart3, Bell, ClipboardCheck, Database, Gauge, GitPullRequest, Info, ListChecks, LogOut, MonitorSmartphone, RefreshCw, ScanLine, SendToBack, Settings, ShieldCheck, Ticket, UsersRound, Wifi, X } from "lucide-react";
...
export default function OperationalMenuPanel({ onClose }) {
  ...
  const active = () => {
    ...
    if (panel === "about") return <Section title="About ScanOps"><p className="text-sm font-semibold text-foreground">Invyra ScanOps</p><p className="text-xs text-muted-foreground">Stage AH — Inventory Desktop Sync API Contract preview layer.</p></Section>;
    return null;
  };
