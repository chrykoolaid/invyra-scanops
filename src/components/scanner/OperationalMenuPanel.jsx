...
import { Activity, BadgeHelp, Bell, ClipboardCheck, Gauge, Info, LogOut, MonitorSmartphone, RefreshCw, ScanLine, Settings, ShieldCheck, Ticket, Wifi, X } from "lucide-react";
...
const MENU_SECTIONS = [
  { title: "Daily Controls", items: [
    ...
    { id: "device", label: "Device Status", icon: MonitorSmartphone, description: "Session, scanner, and store context" },
...
function TicketsPanel() {
  ...
  return <Section title="Shelf Ticket Queue Status"><p className="text-xs text-muted-foreground mb-3">Scanner-created ticket batches waiting for desktop preview/print handoff.</p><div className="grid grid-cols-4 gap-2"><SmallStat label="Total" value={tickets.length} /><SmallStat label="Waiting" value={count(SYNC_STATUSES.QUEUED) + count(SYNC_STATUSES.LOCAL_SAVED)} /><SmallStat label="Synced" value={count(SYNC_STATUSES.SYNCED)} /><SmallStat label="Issue" value={count(SYNC_STATUSES.FAILED) + count(SYNC_STATUSES.CONFLICT)} /></div>{tickets.length === 0 ? <p className="mt-3 rounded-xl border border-dashed border-border p-4 text-sm text-muted-foreground text-center">No shelf ticket batches queued.</p> : <div className="mt-3 space-y-2">{tickets.slice(0, 8).map((entry) => <article key={entry.id} className="rounded-xl border border-border bg-card p-3"><p className="text-sm font-bold truncate">{entry.payloadSummary}</p><p className="text-xs text-muted-foreground truncate">{entry.statusLabel} · {entry.id}</p></article>)}</div>}</Section>;
}
...
export default function OperationalMenuPanel({ onClose }) {
  ...
  const active = () => {
    ...
    if (panel === "about") return <Section title="About ScanOps"><p className="text-sm font-semibold text-foreground">Invyra ScanOps</p><p className="text-xs text-muted-foreground">Stage J — Header Operational Menu + Role / Device / Audit Hardening.</p></Section>;
    return null;
  };
  return <div className="fixed inset-0 z-50 overflow-hidden" role="dialog" aria-modal="true"><button className="absolute inset-0 bg-foreground/30" aria-label="Close operational menu" onClick={onClose} /><aside className="absolute left-0 top-0 h-full w-full max-w-[430px] bg-card border-r border-border shadow-2xl flex flex-col overflow-hidden"><header className="px-4 py-3 border-b border-border flex items-start justify-between gap-3 shrink-0"><div className="min-w-0"><p className="text-xs font-semibold text-primary uppercase tracking-wide">Operational Menu</p><h2 className="text-base font-bold text-foreground">Invyra ScanOps</h2><p className="text-xs text-muted-foreground truncate">{session.actorName} · {session.actorRole} · {session.departmentName}</p></div><button type="button" onClick={onClose} className="w-10 h-10 rounded-xl bg-secondary flex items-center justify-center active:bg-border" aria-label="Close menu"><X className="w-5 h-5" /></button></header><div className="px-4 py-3 border-b border-border bg-background/70 shrink-0"><div className="grid grid-cols-4 gap-2"><SmallStat label="Queued" value={summary.queued} /><SmallStat label="Review" value={summary.needsReview} /><SmallStat label="Failed" value={summary.failed} /><SmallStat label="Synced" value={summary.synced} /></div></div><main className="flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-4">{panel !== "menu" && <button type="button" onClick={() => setPanel("menu")} className="text-sm font-semibold text-primary active:opacity-70">← Back to menu</button>}{panel === "menu" ? <div className="space-y-4">{lastAction && <div className="rounded-xl border border-accent/30 bg-accent/10 p-3 flex items-start gap-2"><Bell className="w-4 h-4 text-accent mt-0.5 shrink-0" /><p className="text-xs font-semibold text-foreground">{lastAction}</p></div>}{MENU_SECTIONS.map((section) => <Section key={section.title} title={section.title}><div className="grid grid-cols-1 gap-2">{section.items.map((item) => { const Icon = item.icon; return <button key={item.id} type="button" onClick={() => action(item)} className="w-full rounded-xl border border-border bg-card p-3 text-left flex items-center gap-3 active:scale-[0.99] active:bg-secondary transition-all"><span className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center shrink-0"><Icon className="w-5 h-5" /></span><span className="min-w-0"><span className="block text-sm font-bold text-foreground">{item.label}</span><span className="block text-xs text-muted-foreground truncate">{item.description}</span></span></button>; })}</div></Section>)}</div> : active()}</main></aside></div>;
}
