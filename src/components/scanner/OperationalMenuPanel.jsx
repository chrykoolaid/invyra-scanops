...
import { Activity, BadgeHelp, BadgePercent, BarChart3, Bell, ClipboardCheck, ClipboardList, Database, Gauge, GitPullRequest, Info, LayoutDashboard, ListChecks, LogOut, MonitorSmartphone, RefreshCw, ScanLine, SendToBack, Settings, ShieldCheck, Ticket, UsersRound, Wifi, X } from "lucide-react";
...
const MENU_SECTIONS = [
  { title: "Operations", items: [
    ...
    { id: "desktop_sync_contract_route", label: "Desktop Sync Contract", icon: SendToBack, description: "Payload validation and desktop response preview" },
    { id: "store_ops_dashboard_route", label: "Store Ops Dashboard", icon: LayoutDashboard, description: "Exception visibility and local command center" },
    { id: "pilot_readiness_route", label: "Pilot Readiness", icon: ClipboardList, description: "UAT scripts and release-gate proof" },
...
export default function OperationalMenuPanel({ onClose }) {
  ...
  const action = (item) => {
    ...
    if (item.id === "desktop_sync_contract_route") { navigate("/desktop-sync-contract"); onClose(); return; }
    if (item.id === "store_ops_dashboard_route") { navigate("/store-ops-dashboard"); onClose(); return; }
    if (item.id === "pilot_readiness_route") { navigate("/pilot-readiness"); onClose(); return; }
  ...
  const active = () => {
    ...
    if (panel === "about") return <Section title="About ScanOps"><p className="text-sm font-semibold text-foreground">Invyra ScanOps</p><p className="text-xs text-muted-foreground">Stage AJ — Pilot Readiness / UAT Hardening Pack on top of the Stage AI command center.</p></Section>;
    return null;
  };
