...
import { INVENTORY_SNAPSHOT_ITEMS } from "../../lib/inventorySnapshot";
import { calculateDaysToExpiry } from "../../lib/scanOpsMarkdownApproval";

const OVERSTOCK_RATIO = 2.5; // shelf stock > this multiple of minimumStock → overstocked

function categorizeSuggestion(item) {
  const days = calculateDaysToExpiry(item.expiryDate);
  const shelf = item.shelfStock ?? 0;
  const min = item.minimumStock ?? 1;

  if (!item.markdownEligible) return null;
  if (item.wasteReviewRequired) return null;

  // Expired → not markdown eligible
  if (days != null && days < 0) return null;

  if (days != null && days === 0) {
    return { urgency: "critical", tag: "Expires today", tagColor: "bg-red-100 text-red-700", reason: "short_dated", suggestedPercent: "50", days };
  }
  if (days != null && days <= 1) {
    return { urgency: "high", tag: `Expires in ${days + 1}d`, tagColor: "bg-red-100 text-red-700", reason: "short_dated", suggestedPercent: "40", days };
  }
  if (days != null && days <= 3) {
    return { urgency: "high", tag: `Expires in ${days}d`, tagColor: "bg-amber-100 text-amber-700", reason: "short_dated", suggestedPercent: "30", days };
  }
  if (days != null && days <= 7) {
    return { urgency: "medium", tag: `Expires in ${days}d`, tagColor: "bg-amber-100 text-amber-700", reason: "short_dated", suggestedPercent: "25", days };
  }
  if (shelf > min * OVERSTOCK_RATIO) {
    return { urgency: "medium", tag: "Overstocked", tagColor: "bg-blue-100 text-blue-700", reason: "overstock", suggestedPercent: "15", days };
  }
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <p className="break-words text-sm font-black leading-tight text-foreground">{item.name}</p>
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-black ${cat.tagColor}`}>{cat.tag}</span>
                  </div>
                  <p className="mt-1 text-[11px] font-semibold text-muted-foreground">
                    {item.shelfLocation} · {item.department}
                  </p>
                  <div className="mt-2 flex items-center gap-3">
                    <span className="flex items-center gap-1 text-[11px] font-bold text-muted-foreground">
                      <Package className="h-3 w-3" />
                      {item.shelfStock ?? "—"} {item.unitType} shelf
                    </span>
                    {item.currentPrice && (
                      <span className="text-[11px] font-bold text-muted-foreground">₱{item.currentPrice} → ₱{(item.currentPrice * (1 - Number(cat.suggestedPercent) / 100)).toFixed(2)}</span>
                    )}
