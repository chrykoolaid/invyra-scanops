...
function HeaderMetaPill({ value }) {
  ...
  return (
    <span className="max-w-[7.5rem] truncate rounded-full bg-secondary px-2 py-0.5 text-[10px] font-bold text-muted-foreground">
      {value}
    </span>
  );
}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5" aria-label="Current ScanOps session context">
            {metaItems.map((item) => <HeaderMetaPill key={item} value={item} />)}
          </div>
        </div>
        <SyncStatusChip />
      </div>
