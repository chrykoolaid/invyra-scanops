...
import { useScanOpsSessionControl, setScanOpsSessionControl } from "../lib/scanOpsSessionControl";
...
function DeviceWorkspace({ session, network, onBack }) {
  const battery = useBatteryStatus();
  const batteryValue = battery && battery.supported ? `${battery.level}%${battery.charging ? " · Charging" : ""}` : "Unavailable";
  const chargingValue = battery && battery.supported ? (battery.charging ? "Yes" : "No") : "Unknown";
  return <WorkspaceShell icon={Smartphone} label="Device Workspace" title="Device" helper="Physical handheld, Wi-Fi, battery, storage, and hardware status." onBack={onBack}><WorkspaceSection title="Current Device" helper="Read-only hardware and operating context."><InfoGrid rows={[["Device ID", session.deviceId], ["Store / Dept", session.departmentName], ["App Version", "ScanOps Preview"], ["Uptime", "Active shift"]]} /></WorkspaceSection><WorkspaceSection title="Wi-Fi" helper="Android owns Wi-Fi joining, passwords, and network selection. ScanOps only shows status."><InfoGrid rows={[["Status", "Connected"], ["Network", network], ["Signal", "Strong"], ["IP Address", "Auto assigned"]]} /><SettingRow icon={Wifi} title="Wi-Fi Details" helper="SSID, IP, MAC, and signal strength." value="Device owned" /></WorkspaceSection><WorkspaceSection title="Power & Storage" helper={battery && battery.supported ? "Live battery telemetry from the device." : "Battery API is not available in this preview. Live values will appear on a real handheld."}><InfoGrid rows={[["Battery", batteryValue], ["Charging", chargingValue], ["Storage", "Healthy"], ["Memory", "Healthy"]]} /></WorkspaceSection><PlannedList items={["Bluetooth", "NFC", "RFID reader", "Camera health", "USB", "GPS", "Docking station", "Charging cradle"]} /></WorkspaceShell>;
}
...
function SessionWorkspace({ session, onBack }) {
  const control = useScanOpsSessionControl();
  const { logout } = useAuth();
  const setTimeoutValue = (value) => setScanOpsSessionControl({ timeout: value });
  const toggleLock = () => {
    if (control.timeout === "Off") setScanOpsSessionControl({ timeout: "15 min", lockEnabled: true });
    else setScanOpsSessionControl({ lockEnabled: !control.lockEnabled });
  };
  const lockActive = control.timeout !== "Off" && control.lockEnabled;
  const handleSignOut = () => {
    try { window.localStorage.removeItem("invyra_scanops_session_locked_v1"); } catch {}
    logout(true);
  };
  return <WorkspaceShell icon={LockKeyhole} label="Session Workspace" title="Session" helper="Current user, shift, timeout, lock, and sign out controls." onBack={onBack}><WorkspaceSection title="Current Session"><InfoGrid rows={[["User", session.actorName], ["Role", session.actorRole], ["Store / Dept", session.departmentName], ["Device", session.deviceId]]} /></WorkspaceSection><WorkspaceSection title="Idle Timeout" helper="Auto lock or sign-out protects shared handheld devices."><ChoiceRow title="Auto Sign Out" choices={["Off", "15 min", "30 min"]} value={control.timeout} onChange={setTimeoutValue} /><ToggleRow icon={LockKeyhole} title="Session Lock" helper="Lock the scanner on idle instead of signing out." checked={lockActive} onToggle={toggleLock} /><SettingRow icon={UserRound} title="Sign Out" helper="End this scanner user session now." value="Active" onClick={handleSignOut} /></WorkspaceSection><PlannedList items={["PIN unlock", "Biometric unlock", "Shift handover", "Device reservation", "SSO", "MFA"]} /></WorkspaceShell>;
}
