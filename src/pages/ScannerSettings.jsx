...
function SyncWorkspace({ network, syncSummary, navigate, onBack }) { return <WorkspaceShell icon={Database} label="Sync Workspace" title="Sync & Connectivity" helper="Inventory Desktop pairing, bridge status, and queue visibility." onBack={onBack}><WorkspaceSection title="Bridge Status" helper="Wi-Fi status belongs under Device. Inventory bridge status belongs here."><InfoGrid rows={[["Bridge", syncSummary.issue ? "Attention" : "Ready"], ["Pending Queue", syncSummary.pending], ["Network", network], ["Environment", "Training / Live aware"]]} /></WorkspaceSection><WorkspaceSection title="Actions"><SettingRow icon={MonitorSmartphone} title="Desktop Pairing" helper="Pair this scanner with the Inventory Desktop bridge." value="Open" action onClick={() => navigate("/sync-handoff")} /><SettingRow icon={RefreshCw} title="Retry Sync" helper="Retry queued handoff records from the sync workspace." value="Open" action onClick={() => navigate("/sync-queue")} /><SettingRow icon={ShieldCheck} title="Environment" helper="Live / Training status is shown, not changed here." value="Read-only" /></WorkspaceSection><PlannedList items={["Multi-site sync", "Cloud backup status", "Conflict resolution", "Incremental sync", "Connection history", "Sync analytics"]} /></WorkspaceShell>; }
