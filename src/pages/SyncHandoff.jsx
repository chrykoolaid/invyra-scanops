    ...
    <div className="flex gap-1.5 overflow-x-auto pb-0.5">
      {tabs.map((tab) => (
        <button
          ...
          className={`${TAB_BUTTON} ${active === tab.id ? "bg-primary text-primary-foreground border-primary" : "bg-card text-foreground border-border"}`}
        >
          {tab.label}
        </button>
      ))}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-sm font-black text-foreground break-words">{title}</h2>
            {helper && <p className="mt-0.5 text-xs font-semibold leading-snug text-muted-foreground break-words">{helper}</p>}
          </div>
        </div>
...
function InfoRow({ label, value }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-border last:border-b-0 py-3">
      <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{label}</span>
      <span className="text-sm font-black text-foreground text-right break-words">{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3">
      <p className="text-xs font-black leading-snug text-amber-800">
        TEST/TRAINING handoff review only. LIVE transport is not active. No inventory, stock, price, accounting, or workflow mutation occurs here.
      </p>
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      <Section icon={Database} title="Queue Status" helper="No default mock records are loaded. Future TEST/TRAINING queue records will appear here before desktop handoff.">
        <div className="grid grid-cols-2 gap-2">
          <Metric label="Pending" value={EMPTY_QUEUE.pending} />
          <Metric label="Review" value={EMPTY_QUEUE.review} />
          <Metric label="Ready" value={EMPTY_QUEUE.ready} />
          <Metric label="Discarded" value={EMPTY_QUEUE.discarded} />
        </div>
      </Section>
      ...
      <Section icon={CheckCircle2} title="No sync records yet" helper="Records created in TEST or TRAINING mode will appear here before desktop handoff.">
        <div className="rounded-2xl bg-secondary/60 px-4 py-5 text-center">
          ...
          <p className="mt-1 text-xs font-semibold text-muted-foreground">There are no pending, review, ready, or discarded records by default.</p>
        </div>
      </Section>
    </div>
  );
...
function SetupTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Section icon={Settings2} title="Handoff Endpoint" helper="Configure the future TEST/TRAINING desktop destination. Saving this form does not start transport or send records.">
        <Field label="Desktop IP Address" placeholder="e.g. 192.168.1.50" value={config.ipAddress || ""} onChange={(value) => update("ipAddress", value)} helper="Static IP of the desktop machine running the inventory system." />
        <Field label="Port" placeholder="e.g. 8080" value={config.port || ""} onChange={(value) => update("port", value)} helper="Port the future desktop handoff listener will use." />
        <Field label="Store / Location ID" placeholder="e.g. STORE-01" value={config.storeId || ""} onChange={(value) => update("storeId", value)} helper="Used to scope future records to this store location." />
        <Field label="Endpoint path optional" placeholder="e.g. /scanops/handoff" value={config.path || ""} onChange={(value) => update("path", value)} helper="Leave blank to use the root path." />
        <button type="button" onClick={save} className={BTN_PRIMARY}>
          <Save className="h-4 w-4" />{saved ? "Saved locally" : "Save Configuration"}
        </button>
      </Section>

      {endpointPreview && (
        <Section icon={FileJson2} title="Current Endpoint Preview" helper="Stored locally for future TEST/TRAINING handoff setup only.">
          <p className="font-mono text-sm font-bold text-foreground break-all">{endpointPreview}</p>
        </Section>
      )}

      <Section icon={ShieldAlert} title="Setup Guide" helper="Use this as a checklist only. The real listener remains a later controlled step.">
        <div className="space-y-2">
          {[
            ["1. Connect to store WiFi", "Ensure the handheld and desktop are on the same network."],
            ["2. Set desktop IP", "Assign a static IP to the desktop running Inventory."],
            ["3. Enter IP + port", "Save the TEST/TRAINING endpoint configuration."],
            ["4. Start desktop listener later", "Do this only after the approved bridge listener exists."],
          ].map(([title, desc]) => (
  ...
  return (
    <div className="space-y-4">
      ...
      <Section icon={GitBranch} title="Contract Status" helper="Payload envelope inspection shell. No fake payload is selected by default.">
        <div className="grid grid-cols-2 gap-2">
          <Metric label="Mode" value="Contract Preview" />
          <Metric label="Transport" value="Not connected" />
          <Metric label="Waiting" value="0" />
          <Metric label="Review req." value="0" />
        </div>
      </Section>

      <Section icon={FileJson2} title="No payload selected" helper="Create or select a TEST/TRAINING queue record later to inspect its contract.">
        <div className="rounded-2xl bg-secondary/60 px-4 py-3">
          <InfoRow label="Action" value="Preview only" />
          <InfoRow label="Mutation" value="Blocked" />
          <InfoRow label="Review queue" value="None" />
        </div>
      </Section>
          ...
          {WORKFLOW_SYNC_CONTRACTS.map((contract) => (
            <div key={contract.stage} className="rounded-2xl border border-border bg-background/70 p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="shrink-0 rounded-full border border-border px-2 py-1 text-[10px] font-black uppercase tracking-wide text-muted-foreground">
                  {contract.eventTypes.length} events
                </span>
              </div>
            </div>
          ))}
