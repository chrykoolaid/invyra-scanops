...
const STOCK_OUT_TYPES = [
  { id: "wastage_damage", label: "Wastage / Damage", tone: "border-red-200 bg-red-50 text-red-800", helper: "Unsaleable stock loss", examples: "Damaged, broken, contaminated", defaultReason: "damaged_in_handling", reasonIds: ["damaged_in_handling", "packaging_broken", "spillage", "contamination_risk"] },
  { id: "expired_spoiled", label: "Expired / Spoiled", tone: "border-orange-200 bg-orange-50 text-orange-800", helper: "Expired or spoiled stock", examples: "Expired, spoiled, temperature issue", defaultReason: "expired_out_of_date", reasonIds: ["expired_out_of_date", "spoiled_rotten", "temperature_issue"] },
  ...
  { id: "suspected_theft_loss", label: "Suspected Theft / Loss", tone: "border-amber-200 bg-amber-50 text-amber-800", helper: "Controlled loss review", examples: "Suspected theft, empty pack, tampering", defaultReason: "theft_suspected_theft", noteRequired: true, reasonIds: ["theft_suspected_theft", "seal_tampering", "missing_stock"] },
  { id: "unknown_shrinkage", label: "Unknown Shrinkage", tone: "border-slate-300 bg-slate-50 text-slate-800", helper: "Unexplained stock loss", examples: "Unknown loss, count discrepancy", defaultReason: "unknown_loss", noteRequired: true, reasonIds: ["unknown_loss", "count_discrepancy", "high_value_discrepancy"] },
];
...
function TypeButton({ type, active, onClick }) {
  return (
    <button type="button" onClick={onClick} className={`min-h-24 rounded-3xl border p-3 text-left active:scale-[0.99] ${active ? type.tone : "border-border bg-card text-foreground"}`}>
...
function StepGuide() {
  const steps = [
    ["Scan item", "Identify the product first."],
    ["Classify event", "Waste, theft, damage, loss, or store use."],
    ["Qty + reason", "Add evidence for review."],
    ["Submit queue", "Inventory Desktop reviews and posts."],
  ];

  return (
    <SectionCard className="space-y-3 border-primary/20 bg-primary/5">
      <div>
        <p className="text-xs font-black uppercase tracking-wider text-muted-foreground">Report Stock-Out</p>
        <h2 className="mt-1 text-lg font-black text-foreground">What happened to this stock?</h2>
        <p className="mt-1 text-sm font-bold leading-snug text-muted-foreground">
          Record floor evidence only. ScanOps does not directly adjust stock from this screen.
        </p>
      </div>
      <div className="grid grid-cols-2 gap-2">
        {steps.map(([label, helper], index) => (
          <div key={label} className="rounded-2xl bg-background/80 px-3 py-2">
            <p className="text-[10px] font-black uppercase tracking-wide text-muted-foreground">Step {index + 1}</p>
            <p className="mt-0.5 text-xs font-black text-foreground">{label}</p>
            <p className="mt-0.5 text-[11px] font-bold leading-snug text-muted-foreground">{helper}</p>
          </div>
        ))}
      </div>
    </SectionCard>
...
function QueueCard({ review }) {
  const meta = QUEUE_CLASS_META[review.reasonCode] || {
    label: review.reviewType === "operational_use" ? "Store Use" : review.reviewType === "shrink" ? "Loss Review" : "Wastage / Damage",
...
export default function Waste() {
  ...
  return (
    <PageShell>
      <PageHeader title="Report Stock-Out" subtitle="Waste, theft, damage, loss evidence" />
      <WorkflowHeader
        title="Report Stock-Out"
        subtitle="Scan → classify → qty/reason → queue → submit"
        scanValue={scanValue}
      ...
      <WorkflowMain>
        ...
        {item ? (
            ...
            <SectionCard className="space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-black text-foreground">Classify the stock-out</h2>
                  <p className="mt-1 text-xs font-bold leading-snug text-muted-foreground">Choose the closest reason. Theft and unknown loss require a clear note for review.</p>
                </div>
                <StatusPill>Evidence</StatusPill>
              </div>
              ...
              <QuantityStepper
                ...
                onChange={(value) => {
                  setQuantity(value);
                  if (operatorError?.title === "Quantity missing") setOperatorError(null);
                }}
                unit={unit}
                min={1}
              />
              ...
              <TouchSelect
                ...
                onChange={(value) => {
                  setReasonCode(value);
                  if (operatorError?.title === "Reason required") setOperatorError(null);
                }}
              ...
              <TextInputField
                ...
                placeholder={currentType.noteRequired ? "Required: describe what was observed..." : "Optional note, for example: expired on shelf or opened packaging..."}
              />
          ...
          <EmptyState title="Scan item to report stock-out." helper="Use this for waste, theft, damage, store use, or unexplained loss evidence." />
        )}
        ...
        <SectionCard className="space-y-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs font-bold text-muted-foreground">Submit sends evidence to review. It does not directly post stock.</p>
            </div>
            <StatusPill>Draft</StatusPill>
          </div>

          {currentSession.length ? <div className="space-y-2">{currentSession.map((review) => <QueueCard key={review.reviewId} review={review} />)}</div> : <EmptyState title="Queue empty" helper="Scan an item, classify the event, then add it to the queue." />}
        </SectionCard>
      </WorkflowMain>
      <StickyActions
        ...
        onLeft={() => {
          refreshReviews();
          setInlineMessage("Session queue refreshed.");
        }}
