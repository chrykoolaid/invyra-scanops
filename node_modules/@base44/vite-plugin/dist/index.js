...
const SANDBOX_DEP_CACHE_DIR = "/var/cache/base44-vite-deps";
            ...
            config: ({ mode, root = process.cwd() }) => {
                ...
                return {
                    ...
                    ...(isRunningInSandbox
                        ? {
                            logLevel: "error",
                            // The sandbox /app mount can't rename() dirs from the snapshot
                            // layer (EXDEV), which breaks the dep optimizer's commit step.
                            // Keep the dep cache on the container rootfs instead.
                            cacheDir: SANDBOX_DEP_CACHE_DIR,
                            server: {
                                cors: true,
                                fs: {
                                    // Explicit allow replaces Vite's defaults, so include the
                                    // app root alongside the external cache dir (served via /@fs/).
                                    allow: [root, SANDBOX_DEP_CACHE_DIR],
                                },
